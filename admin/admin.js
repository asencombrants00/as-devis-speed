...
async function sendQuoteToClient() {
  ...
  if (!config.templateId) {
    setStatus($('#quoteSendStatus'), 'Il manque le Template ID EmailJS du mail devis.', 'error');
    return;
  }
  if (!window.emailjs?.send) {
    setStatus($('#quoteSendStatus'), 'EmailJS ne s’est pas chargé. Recharge la page.', 'error');
    return;
  }
  ...
  try {
    ...
    setStatus($('#quoteSendStatus'), 'Préparation du PDF…');

    const client = getRequestsClient();
    const { data: userData, error: userError } = await client.auth.getUser();
    if (userError) throw userError;
    const userId = userData?.user?.id;
    if (!userId) throw new Error('Ta connexion Supabase a expiré. Reconnecte-toi dans « Demandes clientes ».');

    const safeName = String(filename || 'devis.pdf')
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .replace(/[^a-zA-Z0-9._-]+/g, '-');
    const uniquePart = (window.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`);
    uploadedPath = `${userId}/${Date.now()}-${uniquePart}-${safeName}`;

    const pdfBlob = doc.output('blob');
    setStatus($('#quoteSendStatus'), 'Enregistrement privé du devis…');
    const { error: uploadError } = await client.storage
      .from('devis')
      .upload(uploadedPath, pdfBlob, {
    ...
    const { data: signedData, error: signedError } = await client.storage
      .from('devis')
      .createSignedUrl(uploadedPath, 60 * 60 * 24 * 30);
    if (signedError) throw signedError;
    const devisUrl = signedData?.signedUrl || signedData?.signedURL;
    if (!devisUrl) throw new Error('Le lien du devis n’a pas pu être créé.');

    const messageWithLink = `${data.mailBody}\n\nTélécharger votre devis PDF :\n${devisUrl}\n\nCe lien reste valable 30 jours.`;

    setStatus($('#quoteSendStatus'), 'Envoi du mail au client…');
    ...
    setStatus(
      $('#quoteSendStatus'),
      `Devis envoyé à ${data.clientEmail}. Le client reçoit un lien privé valable 30 jours.`,
      'success'
    );
  } catch (error) {
    if (uploadedPath) {
      try {
        const client = getRequestsClient();
        await client.storage.from('devis').remove([uploadedPath]);
      } catch (cleanupError) {
        console.warn('Nettoyage du devis non envoyé impossible :', cleanupError);
      }
    }
    setStatus($('#quoteSendStatus'), `Envoi impossible : ${error.text || error.message || error}`, 'error');
  } finally {
