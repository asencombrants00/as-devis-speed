...
async function loadRepositoryData() {
  ...
  parsed.homeVideos ||= { enabled: true, intervalSeconds: 8, items: [] };
  parsed.homeVideos.items ||= [];
...
function currentHomeVideos() {
  if (!state.data) return { enabled: true, intervalSeconds: 8, items: [] };
  state.data.homeVideos ||= { enabled: true, intervalSeconds: 8, items: [] };
  state.data.homeVideos.items ||= [];
...
function homeVideoItems() {
  return currentHomeVideos().items;
}

function canDeleteHomeVideoFile(src) {
  return String(src || '').startsWith('assets/home-videos/');
}

function renderHomeVideosAdmin() {
  const root = $('#homeVideosList');
  if (!root || !state.data) return;
  const settings = currentHomeVideos();
  if ($('#homeVideosEnabled')) $('#homeVideosEnabled').checked = settings.enabled !== false;
  if ($('#homeVideosInterval')) $('#homeVideosInterval').value = Math.max(4, Math.min(30, Number(settings.intervalSeconds || 8)));

  const videos = [...homeVideoItems()].sort((a, b) => Number(a.order || 0) - Number(b.order || 0));
  if (!videos.length) {
    root.innerHTML = '<div class="empty-state">Aucune vidéo d’accueil. Utilise le formulaire ci-dessus pour en ajouter une.</div>';
    return;
  }

  root.innerHTML = videos.map((video, index) => `
    <article class="panel home-video-admin-card" data-home-video-id="${escapeHtml(video.id)}">
      <div class="home-video-admin-preview">
        <video muted loop playsinline controls preload="metadata"><source src="../${escapeHtml(video.src)}"></video>
      </div>
      <div class="home-video-admin-fields">
        <label>Titre affiché<input data-home-video-title="${escapeHtml(video.id)}" value="${escapeHtml(video.title || '')}"></label>
        <label class="checkbox-label"><input data-home-video-visible="${escapeHtml(video.id)}" type="checkbox" ${video.visible === false ? '' : 'checked'}> Vidéo visible sur la page d’accueil</label>
        <div class="home-video-path">${escapeHtml(video.src)}</div>
        <div class="home-video-admin-actions">
          <button class="button secondary small" type="button" data-home-video-move="up" data-home-video-id="${escapeHtml(video.id)}" ${index === 0 ? 'disabled' : ''}>↑ Monter</button>
          <button class="button secondary small" type="button" data-home-video-move="down" data-home-video-id="${escapeHtml(video.id)}" ${index === videos.length - 1 ? 'disabled' : ''}>↓ Descendre</button>
          <button class="button danger small" type="button" data-home-video-delete="${escapeHtml(video.id)}">Supprimer</button>
        </div>
        <div class="home-video-admin-replace">
          <label>Remplacer le fichier<input data-home-video-file="${escapeHtml(video.id)}" type="file" accept="video/mp4,video/webm,video/quicktime"></label>
          <button class="button secondary" type="button" data-home-video-replace="${escapeHtml(video.id)}">Remplacer</button>
        </div>
      </div>
    </article>`).join('');

  $$('[data-home-video-title]', root).forEach(input => input.addEventListener('input', () => {
    const video = homeVideoItems().find(item => item.id === input.dataset.homeVideoTitle);
    if (!video) return;
    video.title = input.value;
    markDirty();
  }));
  $$('[data-home-video-visible]', root).forEach(input => input.addEventListener('change', () => {
    const video = homeVideoItems().find(item => item.id === input.dataset.homeVideoVisible);
    if (!video) return;
    video.visible = input.checked;
    markDirty();
  }));
  $$('[data-home-video-move]', root).forEach(button => button.addEventListener('click', () => moveHomeVideo(button.dataset.homeVideoId, button.dataset.homeVideoMove)));
  $$('[data-home-video-delete]', root).forEach(button => button.addEventListener('click', () => deleteHomeVideo(button.dataset.homeVideoDelete)));
  $$('[data-home-video-replace]', root).forEach(button => button.addEventListener('click', () => replaceHomeVideo(button.dataset.homeVideoReplace)));
}

function reorderHomeVideos() {
  homeVideoItems().forEach((item, index) => { item.order = index; });
}

function moveHomeVideo(videoId, direction) {
  const items = homeVideoItems();
  const ordered = [...items].sort((a, b) => Number(a.order || 0) - Number(b.order || 0));
  const index = ordered.findIndex(item => item.id === videoId);
  const target = direction === 'up' ? index - 1 : index + 1;
  if (index < 0 || target < 0 || target >= ordered.length) return;
  [ordered[index], ordered[target]] = [ordered[target], ordered[index]];
  currentHomeVideos().items = ordered;
  reorderHomeVideos();
  markDirty();
  renderHomeVideosAdmin();
}
...
async function addHomeVideo() {
  const fileInput = $('#newHomeVideoFile');
  const titleInput = $('#newHomeVideoTitle');
  const file = fileInput?.files?.[0];
  if (!file) { setStatus($('#homeVideosStatus'), 'Choisis un fichier vidéo.', 'error'); return; }
  if (!file.type.startsWith('video/')) { setStatus($('#homeVideosStatus'), 'Le fichier choisi n’est pas une vidéo.', 'error'); return; }
  if (file.size > MAX_VIDEO_BYTES) { setStatus($('#homeVideosStatus'), 'La vidéo dépasse 25 Mo. Il faut la compresser.', 'error'); return; }

  setBusy(true, `Envoi de ${file.name} sur GitHub…`);
  try {
    ...
    const path = `assets/home-videos/${filename}`;
    await uploadRepositoryFile(path, file, `Ajout de ${file.name} aux vidéos d’accueil`);
    const items = homeVideoItems();
    items.push({
      id: uid('home-video'),
    ...
    setStatus($('#homeVideosStatus'), 'Vidéo ajoutée et publiée.', 'success');
    renderHomeVideosAdmin();
  } catch (error) {
    setStatus($('#homeVideosStatus'), `Ajout impossible : ${error.message}`, 'error');
  } finally {
    setBusy(false);
  }
}

async function replaceHomeVideo(videoId) {
  const video = homeVideoItems().find(item => item.id === videoId);
  const input = document.querySelector(`[data-home-video-file="${CSS.escape(videoId)}"]`);
  const file = input?.files?.[0];
  if (!video || !file) { setStatus($('#homeVideosStatus'), 'Choisis la nouvelle vidéo à utiliser.', 'error'); return; }
  if (!file.type.startsWith('video/')) { setStatus($('#homeVideosStatus'), 'Le fichier choisi n’est pas une vidéo.', 'error'); return; }
  if (file.size > MAX_VIDEO_BYTES) { setStatus($('#homeVideosStatus'), 'La vidéo dépasse 25 Mo. Il faut la compresser.', 'error'); return; }

  const oldPath = video.src;
  setBusy(true, `Remplacement par ${file.name}…`);
  try {
    const extension = file.name.includes('.') ? file.name.split('.').pop().toLowerCase() : 'mp4';
    const filename = `${Date.now()}-${slugify(file.name.replace(/\.[^.]+$/, ''))}.${extension}`;
    const path = `assets/home-videos/${filename}`;
    await uploadRepositoryFile(path, file, `Remplacement d’une vidéo d’accueil par ${file.name}`);
    video.src = path;
    markDirty();
    setBusy(false);
    await saveRepositoryData('Remplacement d’une vidéo sur la page d’accueil');
    if (canDeleteHomeVideoFile(oldPath)) await deleteRepositoryFile(oldPath, 'Suppression de l’ancienne vidéo d’accueil');
    setStatus($('#homeVideosStatus'), 'Vidéo remplacée et publiée.', 'success');
    renderHomeVideosAdmin();
  } catch (error) {
    setStatus($('#homeVideosStatus'), `Remplacement impossible : ${error.message}`, 'error');
  } finally {
    setBusy(false);
  }
}

async function deleteHomeVideo(videoId) {
  const items = homeVideoItems();
  const video = items.find(item => item.id === videoId);
  if (!video || !confirm(`Supprimer la vidéo « ${video.title || 'sans titre'} » de la page d’accueil ?`)) return;
  setBusy(true, 'Suppression de la vidéo…');
  try {
    currentHomeVideos().items = items.filter(item => item.id !== videoId);
    reorderHomeVideos();
    markDirty();
    setBusy(false);
    await saveRepositoryData('Suppression d’une vidéo de la page d’accueil');
    if (canDeleteHomeVideoFile(video.src)) await deleteRepositoryFile(video.src, 'Suppression du fichier vidéo d’accueil');
    setStatus($('#homeVideosStatus'), 'Vidéo supprimée.', 'success');
    renderHomeVideosAdmin();
  } catch (error) {
    setStatus($('#homeVideosStatus'), `Suppression impossible : ${error.message}`, 'error');
  } finally {
    setBusy(false);
  }
}
...
async function sendQuoteToClient() {
  ...
  try {
    ...
    if (!state.requestsConnected) throw new Error('Ouvre d’abord la rubrique « Demandes clientes » et connecte-toi à Supabase.');
    ...
    if (!accessToken) throw new Error('Ta session Supabase a expiré. Reconnecte-toi dans « Demandes clientes ».');

    const { url, key } = getRequestsConfig();
    const pdfDataUrl = doc.output('datauristring');
    const pdfBase64 = String(pdfDataUrl).split(',')[1] || '';
    if (!pdfBase64) throw new Error('Le PDF n’a pas pu être préparé.');
    ...
    setStatus($('#quoteSendStatus'), `Devis envoyé à ${data.clientEmail} avec le PDF en pièce jointe.`, 'success');
  } catch (error) {
...
function initialize() {
  ...
  $('#addHomeVideoButton')?.addEventListener('click', addHomeVideo);
  $('#homeVideosEnabled')?.addEventListener('change', event => { currentHomeVideos().enabled = event.target.checked; markDirty(); });
  $('#homeVideosInterval')?.addEventListener('input', event => { currentHomeVideos().intervalSeconds = Math.max(4, Math.min(30, Number(event.target.value || 8))); markDirty(); });
