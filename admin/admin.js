...
async function sendQuoteToClient() {
  try {
    ...
    if (!confirm(`Préparer le devis ${data.quoteNumber} pour ${data.clientEmail} ?`)) return;

    const button = $('#sendQuoteButton');
    if (button) button.disabled = true;
    setStatus($('#quoteSendStatus'), 'Téléchargement du PDF…');

    doc.save(filename);

    const mailBody = `${data.mailBody}\n\nLe devis PDF est prêt. Pense à ajouter le fichier « ${filename} » en pièce jointe avant d’envoyer ce mail.`;
    const mailto = `mailto:${encodeURIComponent(data.clientEmail)}?subject=${encodeURIComponent(data.mailSubject)}&body=${encodeURIComponent(mailBody)}`;

    setStatus(
      $('#quoteSendStatus'),
      `Le PDF « ${filename} » a été téléchargé. Ajoute-le au mail qui va s’ouvrir, puis appuie sur Envoyer.`,
      'success'
    );
    ...
    setStatus($('#quoteSendStatus'), `Préparation impossible : ${error.message || error}`, 'error');
  } finally {
    const button = $('#sendQuoteButton');
