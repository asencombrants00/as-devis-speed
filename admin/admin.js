...
async function sendQuoteToClient() {
  ...
  try {
    ...
    if (!confirm(`Envoyer maintenant le devis ${data.quoteNumber} à ${data.clientEmail} ?`)) return;
    setStatus($('#quoteSendStatus'), 'Envoi du devis en cours…');
    ...
    setStatus($('#quoteSendStatus'), `Devis envoyé à ${data.clientEmail}.`, 'success');
  } catch (error) {
